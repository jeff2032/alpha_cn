...
THEME_CLUSTERS: dict[str, list[str]] = {
    ...
    "通信设备": ["通信", "CPO", "光模块", "5G", "6G", "网络设备", "通信服务", "电子设备制造"],
...
def _candidate_bucket_sections(candidates: pd.DataFrame, *, top: int) -> list[tuple[str, str, pd.DataFrame]]:
    ...
    return [
        ...
        (
            "A3 主线趋势延续池",
            "主线仍强、风险干净的 A3 才是主攻；高波动 A3 只看分歧承接，避免高开过热追买。",
            trend,
        ),
        (
            "B2s 主线突发观察池",
            "这里专门承接近期容易 miss 的低位主线突发样本；不是直接买点，先看开盘承接、持续性和公告/情绪核验。",
            surge,
        ),
        (
            "B1/B2 主题扩散观察池",
            "B2a/B2b 是主线扩散和主题待确认池；必须看次日持续性、盘中承接和风险核验。",
            theme_watch,
        ),
    ]
