...
影子组合还使用市场环境仓位门：强势 60%、震荡偏强 40%、震荡 20%、防守 10%、未知 0%；市场温度分不高于 25 时保持全现金。当宽基指数出现 1/3/5 日快速下跌时会立即降到防守档，不等待 20/60 日趋势完全转弱。
...
回填严格读取当日已经冻结的研究快照，不用未来字段修饰过去。每天最多冻结 5 只：A2 最多 2 只、A1 最多 2 只、A3 最多 1 只；只允许低风险信号，B1/B2 和中高风险不进入影子组合。有效标的不足时允许少于 5 只或全现金，不为凑数量放宽规则。命令会同时计算 `stateful` 与 `daily_target`，比较收益、最大回撤、交易次数、费用和平均持仓数；主报告默认保存状态化结果。
...
当前动作周期统一为：A1 潜伏观察 10-20 日、A2 主攻 3-5 日、B2 升级观察 3-5 日、A3 短线 1-3 日；B1/B2 只保留为内部观察或对照，不进入影子组合。
