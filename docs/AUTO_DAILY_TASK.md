...
- 运行中的最新状态固定写入 `reports/ops/nightly_prep_current.md`；即使电脑休眠、任务被终止或数据源尚未更新，也能看到最后完成的重试轮次和剩余缺口。
- `data/universe/a_stock.csv` 是当前可交易研究池，不再保存历次股票池并集。历史上市/退市状态由 `security_master_daily` 按日期保存，避免退市代码反复触发行情重试。
